...
parser.add_argument('--output_dir', default='./checkpoints/GrahpCLIP')
parser.add_argument('--checkpoint_name', default='GraphCLIP')
...
def main(args):
  ...
  while True:
    ...
    for batch in train_loader:
      ...
      if len(batch) == 6:
        imgs, objs, _, triples, obj_to_img, _ = batch
      elif len(batch) == 7:
        imgs, objs, _, _, triples, obj_to_img, _ = batch
      else:
        assert False
      ...
      if t % args.print_every == 0:
        print('%d / %d, loss: %.4f' % (t, args.num_iterations, losses['total_loss']))
