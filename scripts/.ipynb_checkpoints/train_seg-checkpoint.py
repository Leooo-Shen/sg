...
def check_model(loader, model, clip_features):
  ...
  with torch.no_grad():
    for batch in loader:
      ...
      masks_for_sum = masks_pred.sigmoid()
      simple_sum = sum_masks(masks_pred, objs, by='sum') # [batch, 1, 64, 64]      

      loss_mask = F.binary_cross_entropy_with_logits(masks_pred, masks.float())
      loss_bbox = F.mse_loss(boxes_pred, boxes)
      criterian = nn.L1Loss()
      loss_seg = criterian(simple_sum, seg_maps)
      total_loss = loss_mask  + loss_bbox + loss_seg * 0.005
...
def sum_masks(masks, objs, by='argmax'):
  '''
  Input: concated object-wise masks
  Return: [batch, M, M] segmentation masks
  '''
  ...
  if by=='sum':
    ...
    for m, obj in zip(mask_split, obj_split):
      ...
      mask_cls = binary_m * obj.unsqueeze(dim=-1).unsqueeze(dim=-1)
      mask_sum.append(mask_cls.sum(dim=0, keepdim=True))
    seg = torch.cat(mask_sum, dim=0)
    
  elif by=='argmax':
    out_lst = []
    for m, obj in zip(mask_split, obj_split):
      mask_idx = torch.argmax(m[:-1], dim=0)
      binary_m = (m>0.5).float()   # binaralize
      mask_cls = binary_m * obj.unsqueeze(dim=-1).unsqueeze(dim=-1)
      
      out = torch.zeros(mask_idx.shape)
      for i in range(mask_idx.shape[0]):
        for j in range(mask_idx.shape[1]):
          cls = mask_idx[i][j]
          out[i][j] = mask_cls[cls][i][j]
      out_lst.append(out)
    seg = torch.stack(out_lst, dim=0).cuda()
  return seg
      ...
      for batch in train_loader:
        ...
        with autocast():
          ...
          boxes_pred, masks_pred = model(objs, triples, obj_to_img=obj_to_img, 
                            boxes_gt=boxes, clip_features=clip_embeddings)
          masks_for_sum = masks_pred.sigmoid()
          simple_sum = sum_masks(masks_for_sum, objs, by='argmax') # [batch, 64, 64]      

#           loss_mask = F.binary_cross_entropy(masks_pred, masks.float())
          loss_mask = F.binary_cross_entropy_with_logits(masks_pred, masks.float())
          loss_bbox = F.mse_loss(boxes_pred, boxes)
          criterian = nn.L1Loss()
#           print('[*]', masks_pred.shape, masks_for_sum.shape)
#           print('[*]', masks_pred.device, masks_for_sum.device)
          loss_seg = criterian(simple_sum, seg_maps)
